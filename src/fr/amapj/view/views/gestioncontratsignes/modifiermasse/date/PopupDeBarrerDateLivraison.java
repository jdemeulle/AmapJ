...
public class PopupDeBarrerDateLivraison extends WizardFormPopup
{
	...
	public PopupDeBarrerDateLivraison(Long idModeleContrat)
	{
		...
		datesBarrees = contratDTO.computeDateBarrees();
	...
	{
		add(()->addFieldInfoGenerales(),()->checkDateBarrees());
		add(()->addFieldSaisieDate(),()->readDateToDebarrer());
	...
	{
		// Titre
		setStepTitle("les dates barrées à rendre actives");
		...
		builder.addCheckBox("Ne pas barrer cette date", "cb",true, 150, e->modeleContratDateToDebarrer.contains(e), null); 
	...
	private void addFieldConfirmation()
	{
		...
		addHtml("Vous allez supprimer le barré sur "+modeleContratDateToDebarrer.size()+" dates de ce contrat:");
		
		addHtml(FormatUtils.puceDate(modeleContratDateToDebarrer, e->e.date));
	...
	protected void performSauvegarder() throws OnSaveException
	{
		...
		{
			contratDTO.enableThisLine(ligDTO.i);
		}
		
		// On sauvegarde en base
		new GestionContratService().updateDateBarreesModeleContrat(contratDTO);
		
		String details = "Liste des dates dé barrées : "+SuiviModificationContrat.listeDate(modeleContratDateToDebarrer,e->e.date);
		SuiviModificationContrat.logModification("Dé barrer une ou plusieurs dates de livraison", contratDTO, details);
