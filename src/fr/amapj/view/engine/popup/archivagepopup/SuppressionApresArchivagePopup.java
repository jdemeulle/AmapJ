...
abstract public class SuppressionApresArchivagePopup extends WizardFormPopup
{
	...
	private void addFieldConfirmation()
	{
		...
		case OUI_SANS_RESERVE:
			addHtml("Ce élément respecte bien toutes les conditions pour pouvoir être supprimé.");
			addHtml("Veuillez cliquer sur Sauvegarder pour le supprimer, ou Annuler pour ne rien faire.");
			return;
		...
		case OUI_AVEC_RESERVE_MAJEURE:
			...
			addHtml("Néanmoins, si vous pensez que ce élement est bien supprimable, vous pouvez le supprimer quand même en saisissant ci dessous le texte <br/>SUPPRESSION<br/> puis en cliquant sur Sauvegarder");
		...
		case NON:
			addHtml("Ce élément ne peut pas être supprimé pour les raisons suivantes : ");
			addHtml(asHtmlList(state.nonSupprimables));
			setBackOnlyMode();
			return;
